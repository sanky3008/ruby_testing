...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:chai) { described_class.new('chai') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(chai.type).to eq('chai')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(chai.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:chai) { described_class.new('chai', 15) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(chai.full?).to eq(false)
      end
    end
  end
end
